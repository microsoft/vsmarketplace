<#
.SYNOPSIS
    Sets up and runs a Private Marketplace quickstart environment for Visual Studio Code.
...
$rootFolderName = if ($RepoBranch -eq 'main') {
    "privatemarketplace-quickstart-vscode"
} else {
    "privatemarketplace-quickstart-vscode-$branchSlug"
}
...
if ($missingPrereqs.Count -gt 0 -or $adminTemplatesNeeded) {
    ...
    if (-not $repoExists) {
        ...
        try {
            ...
            $extractedquicklaunchFolder = Join-Path $tempExtractPath "$repoName-$branchSlug\privatemarketplace\preview\quickstart\vscode"
